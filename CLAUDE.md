...
| **WordPress** | `WP_SITE_URL`, `WP_USERNAME`, `WP_APP_PASSWORD` | wp-publish skill — REST API publishing (configured in `clawdbot.json` skill env, not `.env`) |

Keys are set in `/opt/clawdbot.env` on DO and in `~/.zshrc` locally. WordPress credentials are set in `clawdbot.json` under `skills.entries.wp-publish.env`. Never commit live keys — use `<PLACEHOLDER>` values in templates.
